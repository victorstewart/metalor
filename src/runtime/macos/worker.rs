...
use super::client::HELPER_REQUEST_ENV;
...
pub fn load_request_from_env() -> Result<BuildCellSpec> {
    let request = std::env::var(HELPER_REQUEST_ENV)
        .context("missing required macOS helper request environment variable")?;
...
pub fn build_worker_command(spec: &BuildCellSpec, job: &WorkerJob) -> Result<Command> {
